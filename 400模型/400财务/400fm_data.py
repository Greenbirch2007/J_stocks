...
if __name__ =='__main__':
    lpath = os.getcwd()
    j400_allcode =[]
    excelFile = '{0}\\400_code.xlsx'.format(lpath)
    full_items = read_xlrd(excelFile=excelFile)
    for item in full_items:
        j400_allcode.append(str(item[2])[:-2])
    ...
    for num in range(1, long_count):
        ...
        data = cur.fetchone()
        print(data["name"][-5:-1])
        if data["name"][-5:-1] in j400_allcode:
    ...
    csv_dict_write('{0}\\400js_data.csv'.format(l_path),head,big_list)
    print("数据导出完成～")
